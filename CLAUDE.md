...
This repository holds the observability infrastructure library for Standards Lab's Go Elemental
...
- **Module layout.** One base module is rooted at `github.com/standards-lab/go-observability`,
  holding `Config`, the `Telemetry` lifecycle service, the trace-correlating `slog.Handler`, the
  HTTP server middleware, and the request-ID source function at its root. The `otlp` sub-module
  carries its own `go.mod` and pins the gRPC OTLP exporters; the base module never imports it.
